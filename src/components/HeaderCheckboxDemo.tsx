...
export default function HeaderCheckboxDemo() {
  const gridRef = useRef<GridRef>(null);
  const [localData] = useState(generateDynamicData(200));

  // Header checkbox belongs to the checkbox column (not selectionSettings)
  ...
  return (
    <div style={{ margin: '10px' }}>
      <Provider ripple={true}>
        <h2 style={{ margin: '10px' }}>Header checkbox toggle</h2>

        <div style={{ margin: '10px' }}>
          <Button onClick={() => setHeaderCheckbox((v) => !v)} className='e-primary' style={{ marginRight: '10px' }}>
            {headerCheckbox ? 'Disable header checkbox' : 'Enable header checkbox'}
          </Button>
        ...
        {flag && (
          ...
          >
            <Columns>
              <Column type='checkbox' width='40' headerCheckbox={headerCheckbox}></Column>
